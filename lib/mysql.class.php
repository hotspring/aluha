...
//向数据库发送一条SQL语句
    public function exec($arg1) {
        return mysql_query($arg1, $this->conn);
    }
...
//$db = new db("localhost","root","","db_stat");
/* $query=$db->exec("select * from t_area_info");
  $result=$db->fetch_array($query);   //个数
  while($result=$db->fetch_array($query)){
  print_r($result);
  } */
//$result=$db->select("*","t_area_info","","");
//$result=$db->select("order_id,area_name","t_area_info","",array('limit_start'=>1,'limit'=>10,'order_by'=>'order_id'));
//print_r($result);exit;
//$num=$db->num_rows($result);

/* while($fet = $db->fetch_array($result)){
  $array[]=$fet[0];
  }print_r($array);exit; */
//$fields=$db->delete("t_area_info","order_id=1","");   //对数据进行删除
//$insert=$db->insert('t_area_info','order_id,area_name','1,"山东"');   //插入数据库数据
//$update=$db->update('t_area_info',array('area_name'=>'山东','order_id'=>1),"order_id=1","");
//$str=$db->escape("sdfsfd's");
