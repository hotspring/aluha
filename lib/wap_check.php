...
function mobile_device_detect($iphone=true,$android=true,$opera=true,$mobileredirect=false,$desktopredirect=false){
	...
	switch(true){
		...
		break;
		case (preg_match('/(up.browser|up.link|mmp|symbian|smartphone|midp|wap|vodafone|o2|pocket|mobile|pda|psp|treo)/i',$user_agent));
		$mobile_browser = true;
		break;
...
//获取文件本地地址
function getwapuri() {
	$wapuri = "http://" . $_SERVER ['SERVER_NAME'] . ":" . $_SERVER ['SERVER_PORT'] . $_SERVER [PHP_SELF];
	$wapuri = explode ( '/', $wapuri );
	array_pop ( $wapuri );
	$tempuri = '';
	foreach ( $wapuri as $strtemp ) {
		$tempuri .= "$strtemp/";
	}
	$wapuri = $tempuri . "m/";
	return $wapuri;
